<template>
  <div class="flex flex-col bg-black-800 rounded-tl-lg rounded-tr-lg p-5 text-white">
    <img class='w-20' :src=user.image alt="">
    <h2>{{user.name}}</h2>
    <p>{{user.handle}}</p>
    <p>{{user.description}}</p>
    <p>{{user.following}} following</p>
    <p>{{user.followers}} followers</p>
    
    <div>
        <h2>My Podcasts</h2>
        <div v-for="podcast in podcasts" class="flex flex-row">
            <img class='w-5' :src=podcast.image :alt=podcast.title>
        </div>
    </div>

    <div>
        <h2>Posts</h2>
        <div v-for="post in posts" class="flex flex-row">
            <img class="w-4" :src=user.image alt="">
            <p>{{post.title}}</p>
        </div>
    </div>

  </div>
</template>

<script>

export default {
  data(){
      return{ 
        user: {
            image: 'data:image/jpeg;base64,',
            name: 'Brene Brown',
            handle: '@brenebrown',
            'description': 'Avid skater, Founder, Uber Driver',
            'following': '500',
            'followers': '10k'
        },
        podcasts: [
            {'title': 'The Quest Podcast', 'image': 'https://cdn-images-1.listennotes.com/podcasts/the-quest-with-justin-kan-justin-kan-M4kpaCv_DyE-4HFKRM9ETmS.1400x1400.jpg'},
            {'title': 'Jocko Podcast', 'image': 'https://pbs.twimg.com/profile_images/647606396452646912/WKgBcrEL.jpg'},
            {'title': 'The Gabor Mate Podcast', 'image': 'https://resources.soundstrue.com/wp-content/uploads/2017/03/Gabor-Mate-800x450.jpg'}
        ],
        posts: [
            {'title': '#57 - Justin Mann on Finding Meaning in Startup Land', 'link': '', 'clips':['', '', '','']},
            {'title': 'Find Your Sense of Wellbeing', 'link': '', 'clips':['', '', '','']},
            {'title': 'The Source of all Knowledge', 'link': '', 'clips':['', '', '','']},
            {'title': `Your Grandma's Favorite Tea`, 'link': '', 'clips':['', '', '','']},

        ] 
      }
  }
};
</script>

<style></style>
